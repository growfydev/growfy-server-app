...
export class StorageController {
  ...
  getAuthUrl(@Res() res: Response): void {
    try {
      const url = this.storageService.generateAuthUrl();
  ...
  async oauth2Callback(
    ...
    try {
      if (!code) {
        res.status(HttpStatus.BAD_REQUEST).json({ message: 'Missing code parameter' });
        return;
      }

      // Assuming profileId and service are predefined or fetched dynamically
      const profileId = 1; // Replace with dynamic value as needed
      const service = 'GOOGLE_DRIVE';

      await this.storageService.setCredentials(profileId, service, code);
      res.status(HttpStatus.OK).json({
        message: 'Authentication successful! You can now interact with Google Drive.',
      });
    } catch (error) {
      console.log(error)
      res.status(HttpStatus.INTERNAL_SERVER_ERROR).json({
  ...
  async listFiles(
    ...
    try {
      ...
      const files = await this.storageService.listFiles(profileId, service);
  ...
  async uploadFile(
    ...
    try {
      if (!profileId || !service || !filePath || !mimeType) {
        res.status(HttpStatus.BAD_REQUEST).json({
          message: 'Missing required parameters: profileId, service, filePath, or mimeType',
        });
        return;
      }

      const file = await this.storageService.uploadFile(profileId, service, filePath, mimeType);
